...
var global_data = getApi(global_url);

update("scroll-text").innerHTML =
    "Active Cryptocurrency: " + global_data.data.active_cryptocurrencies +
    ", Market cap change 24h: " + global_data.data.market_cap_change_percentage_24h_usd.toFixed(2) + " %" +
    ", Ended icos: " + global_data.data.ended_icos;
...
function coinDetails() {
    ...
    var historical_url = "https://api.coingecko.com/api/v3/coins/" + coinId + "/market_chart?vs_currency=eur&days=180"
    var historical_data = getApi(historical_url);
    var historical_prices = historical_data.prices;
    var price;

    for (let i = 0; i < 180; i++) {
        price = historical_prices[i][1].toFixed(2);
        console.log(price)
    }
...
var options = {
    ...
    title: {
        text: 'CandleStick Chart last 6 months',
        align: 'left'
    },
    xaxis: {
        type: 'datetime'
...
var chart = new ApexCharts(document.getElementById("my-chart"), options);
chart.render();